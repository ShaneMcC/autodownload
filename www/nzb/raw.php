...
//	$pattern .= '.*href="/browse/category/[^/]+/[^/]+/">([^<]+)</a>'; // Category
	$pattern .= '.*&quot;([^<]+)\.<a href="/browse/nzb/([0-9]+)/">nzb</a>'; // Post ID and Name
...
//		echo "\t\t".'<category>'.trim($matches[$matchid['category']][$j]).'</category>'.CRLF;
		...
		// To save time, only care about stuff that looks like what we searched for,
		// unless $global['GetAllSizes'] is true.
		$getMoreInfo = $global['GetAllSizes'] || strtolower($global['WANT_SEARCH']) == strtolower($name);
		if ($getMoreInfo) {
			$sizeInfo = get_page('http://v3.newzbin.com/browse/nzb/'.$nzbid, $global[$global['username']]['cookies']);
			$size = -1;
			$debug = '';
			if ($sizeInfo !== FALSE) {
				$size = 0;
				if (preg_match_all('#<a href="/browse/file/([0-9]+)/".*<td class="fileSize"> \(<span>(.+)((?:M|K|G)?B)</span>\) </td>#Uims', $sizeInfo['page'], $smatch)) {
					for ($si = 0; $si < count($smatch[0]); $si++) {
						$files[] = $smatch[1][$si];
						$s2 = $smatch[2][$si];
						$debug .= ' | '.$smatch[2][$si].' '.$smatch[3][$si];
						switch ($smatch[3][$si]) {
							case 'GB':
								$s2 = $s2 * 1024;
								break;
