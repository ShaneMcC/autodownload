<?php
	...
	function searchFor($search) {
		...
		$data['age'] = isset($_REQUEST['retention']) ? trim($_REQUEST['retention']) : 300;
		$data['num'] = isset($_REQUEST['limit']) ? trim($_REQUEST['limit']) : 500;

		unset($data['num']);
		// $data['offset'] = isset($_REQUEST['page']) ? $page * $data['limit'] : 0;

		$url = 'http://nzbmatrix.com/api-nzb-search.php?';
		foreach ($data as $k => $v) {
			$url .= '&';
			$url .= $k;
			$url .= '=';
